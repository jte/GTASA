...
uint32_t CKeyGen::GetKey(char const *szString)
{
    return GetKey(szString, strlen(szString));
}

uint32_t CKeyGen::GetKey(char const *szString, int uiLength)
{
    uint32_t key = -1;
...
uint32_t CKeyGen::GetUppercaseKey(char const *szString)
{
    uint32_t key = -1;
...
uint32_t CKeyGen::AppendStringToKey(unsigned int uiKey, char const *szString)
{
    size_t uiLength = strlen(szString);
    for(size_t i = 0; i < uiLength; i++)
    {
