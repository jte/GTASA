...
CPhysical::CPhysical()
{
    ...
    GetMatrixLink()->matrix.SetIdentityMatrix();
	m_mass = 1.0f;
	m_turnMass = 1.0f;
	m_pad2 = 1.0f;
	fAirResistance = 0.1f;
...
{
    if (!m_infiniteMass && !m_0x20)
    {
        if (m_ignoreZComponent)
        {
            force.z = 0.0f;
        }
        // a = F / m
...
void CPhysical::ApplyTurnForce(CVector force, CVector point)
{
    if (!m_disableTurnForce)
    {
        ...
        if (!m_infiniteMass)
        {
            center = GetMatrixLink()->matrix * m_center_of_mass; // object-space -> world-space
        }
        if (m_0x20)
        {
            point.z = 0.0;
...
void CPhysical::ApplyMoveSpeed()
{
    ...
    else
    {
        // Euler integration
        // pos += a * dt
        GetMatrix().m_matrix.pos += m_linearAcceleration * CTimer::GetTimeStep();
    }
}
...
void CPhysical::ApplyTurnSpeed()
{
    ...
    else
    {
        ...
        // w = w0 + w x z
        GetMatrix().m_matrix.right += CrossProduct(dt_avel, GetMatrix().m_matrix.right);
        GetMatrix().m_matrix.up += CrossProduct(dt_avel, GetMatrix().m_matrix.up);
        GetMatrix().m_matrix.at += CrossProduct(dt_avel, GetMatrix().m_matrix.at);
        if (!m_infiniteMass && !m_0x20)
        {
            GetMatrix().m_matrix.pos += CrossProduct(dt_avel, GetMatrix().m_matrix * -m_center_of_mass);
        }
    }
}
...
{
    if (m_applyGravity && !m_0x20)
    {
        if (m_infiniteMass)
        {
            CVector gravity(0.0f, CTimer::GetTimeStep() * m_mass * -0.008f, 0.0f);
            CVector inertiaTensor = GetMatrix().m_matrix * m_center_of_mass;
            ApplyForce(gravity, inertiaTensor, true);
        }
        else if (m_usesCollision)
        {
            m_linearAcceleration.z -= CTimer::GetTimeStep() * 0.008f;
        }
    }
}
...
void CPhysical::ApplyForce(CVector force, CVector point, bool apply_rotation)
{
    ...
    if (!m_disableTurnForce && apply_rotation)
    {
        CVector inertia_tensor = CVector(0.0, 0.0, 0.0);
        float turn_mass = m_turnMass;
        if (m_infiniteMass)
        {
            turn_mass += (m_center_of_mass.z * m_center_of_mass.z) * m_mass * 0.5f;
        }
        else
        {
            inertia_tensor = GetMatrix().m_matrix * m_center_of_mass;
        }
        if (m_0x20)
        {
            force.z = 0.0;
        ...
        // x += (point x force) / I
        m_angularAcceleration += CrossProduct(point, force) / turn_mass;
    }
}
...
bool CPhysical::GetHasCollidedWith(CEntity *entity)
{
    if (bEnableCollision)
    {
        if(m_numCollisions >= 0 && m_numCollisions <= ELEMS_COUNT(m_contactedEntities))
        {
            for(size_t i = 0; i < ELEMS_COUNT(m_contactedEntities); i++)
            {
...
void CPhysical::ApplySpringCollisionAlt(float stiffness, CVector& force, CVector& point, float damp, float bias, CVector& alt, float& dx)
{
    ...
    if (damp > 0.0f)
    {
        if (DotProduct(force, alt) > 0.0f)
        {
            alt *= -1.0f;
        }
        float dt = ClampMax(CTimer::GetTimeStep(), 3.0f);
        dx = damp * dt * m_mass * stiffness * bias * 1.0f/62.5f;
        if (m_0x01)
        {
            dx *= 0.75;
        }
...
{
    if (!m_movingList && !bIsStaticWaitingForCollision)
    {
        m_movingListNode = new CEntryInfoNode;
        m_movingListNode->ptr = this;
        m_movingListNode->next = NULL;
        m_movingListNode->prev = CWorld::ms_listMovingEntityPtrs;

        CPtrNodeDouble *m = new CPtrNodeDouble;
        m->ptr = this;
        m->next = NULL;
        m->prev = CWorld::ms_listMovingEntityPtrs;
        m_moving_list = m;
        CWorld::ms_listMovingEntityPtrs->next = m;
        CWorld::ms_listMovingEntityPtrs = m;
    }
}
