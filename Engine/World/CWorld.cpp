...
void CWorld::FindObjectsInRange(CVector const &vecCenter, float fRadius, bool bIgnoreZ, short *usFoundCount, short usMaxCount, CEntity **pFoundObjectsList, bool bIncludeBuildings, bool bIncludeVehicles, bool bIncludePeds, bool bIncludeObjects, bool bIncludeDummies)
{
    CRect sector;
        ...
        if(pObject->m_nScanCode != CWorld::ms_nCurrentScanCode)
        {
            ...
            if(fDistance < fRadius * fRadius)
            {
                if(*usFoundCount < usMaxCount)
                {
                    ...
                    {
                        pFoundObjectsList[*usFoundCount] = pObject;
                    }
                    (*usFoundCount)++;
                }
            }
...
{
    if(CGarages::IsModelIndexADoor(pEntity->m_nModelIndex))
    {
        return false;
    }
...
void CWorld::Add(CEntity *pEntity)
{
    ...
    if(pEntity->m_nType != ENTITY_TYPE_BUILDING &&
       pEntity->m_nType != ENTITY_TYPE_DUMMY)
    {
...
{
    pEntity->Remove();
    if(pEntity->nType >= 2 && pEntity->nType <= 4)
    {
